...
<script type="text/javascript">
	$('#admin_collectorM_datagrid').datagrid({
		url : '${pageContext.request.contextPath}/collectorAction!datagrid.action',
		fit : true,
		//fitColumns:true,
		border : false,
		pagination : true,
	...
	function searchFun() {
		...
		$('#admin_collectorM_datagrid').datagrid('load',
				serializeObject($('#admin_collectorM_searchForm')));
	}
	function append_collectorFun() {
		$('#admin_collectorM_datagrid').datagrid('uncheckAll').datagrid(
		'unselectAll').datagrid('clearSelections');
			$('<div></div>').dialog ({
					id : 'addCollectorDialog',
					href : '${pageContext.request.contextPath}/admin/collectorAdd.jsp',
					width : 270,
					height : 150,
					modal : true,
					title : '添加采集设备',
					buttons : [ {
						...
						handler : function() {
							...
							$('#admin_collectorAdd_addForm')
									.form(
											'submit',
											{
												url : '${pageContext.request.contextPath}/collectorAction!add.action',
												success : function(
														result) {
													try {
														...
														if (r.success) {
															...
															$('#addCollectorDialog').dialog('destroy');
														}
														$.messager
																.show({
																	title : '提示',
					...
					onClose : function() {
						$('#addCollectorDialog').dialog('destory');
					}
				});
	}
	function remove_collectorFun() {
		var rows = $('#admin_collectorM_datagrid').datagrid('getChecked');
		//console.info(rows);
		var ids = [];
		var names=[];
					...
					.confirm(
							...
							function(r) {
								if (r) {
									...
									$
											.ajax({
												url : '${pageContext.request.contextPath}/collectorAction!remove.action',
												data : {
													ids : ids.join(','),
													names:names.join(',')
												},
...
<div id="admin_collectorM_layout" class="easyui-layout"
	data-options="fit:true,border:false">
	<div data-options="region:'north',border:false,title:'查询'"
		style="height:55px;">
		<form id="admin_collectorM_searchForm">
			采集设备名称(包含)：<input name="name" /> <a href="#" class="easyui-linkbutton"
	...
	<div data-options="region:'center',border:false">
		<table id="admin_collectorM_datagrid">
		</table>
	</div>
</div>
<!--  
<div id="admin_collectorM_addCollectorDialog" class="easyui-dialog"
	style="width:500px;height:200px;"
	data-options="closed:true,modal:true,title:'添加用戶',buttons:[{
				text:'增加',
				iconCls:'icon-add',
				handler:function(){
					$('#admin_collectorM_addForm').form('submit',{
						url:'${pageContext.request.contextPath}/collectorAction!add.action',
						success : function(r) {
						var obj = jQuery.parseJSON(r);
						if (obj.success) {
							$('#admin_collectorM_datagrid').datagrid('insertRow',{
								index:0,
								row:obj.obj
							});
							$('#admin_collectorM_addCollectorDialog').dialog('destroy');
						}
						$.messager.show({
							title : '提示',
							msg : obj.msg
	...
	<form id="admin_collectorM_addForm" method="post">
		<table>
			<tr>
				<!--  <th>编号：</th>
